...
class StdinConsole {
  ...
  async _dispatch(input) {
    ...
    switch (cmd) {
      ...
      case 'players':         return this._cmdPlayers(args);
      case 'player':          return this._cmdPlayer(args);
      case 'online':          return this._cmdOnline();
      case 'search':          return this._cmdSearch(args);
      case 'activity':        return this._cmdActivity(args);
      case 'chat':            return this._cmdChat(args);
      case 'state':           return this._cmdState(args);
      case 'state.get':       return this._cmdStateGet(args);
      case 'state.set':       return this._cmdStateSet(args);
      case 'state.delete':    return this._cmdStateDelete(args);
      case 'state.list':      return this._cmdStateList();
      case 'stats':           return this._cmdStats();
      case 'tables':          return this._cmdTables();
      case 'sql':             return this._cmdSql(args, input);
      case 'world':           return this._cmdWorld();
      case 'clans':           return this._cmdClans();
      case 'vehicles':        return this._cmdVehicles();
      default:
...
`.trim());
  ...
  _cmdPlayer(args) {
    if (!this._db) { this._print('No database available.'); return; }
    if (args.length === 0) { this._print('Usage: player <steamId>'); return; }
    ...
    } catch { /* alias table may not exist */ }
  }

  _cmdOnline() {
    if (!this._db) { this._print('No database available.'); return; }
    const players = this._db.getOnlinePlayers();
    if (players.length === 0) { this._print('No players online.'); return; }
  ...
  _cmdSearch(args) {
    if (!this._db) { this._print('No database available.'); return; }
    if (args.length === 0) { this._print('Usage: search <name>'); return; }
    const query = args.join(' ');

    try {
      const results = this._db.searchPlayersByName(query);
      if (!results || results.length === 0) { this._print(`No players found matching "${query}".`); return; }
  ...
  _cmdActivity(args) {
    ...
    try {
      ...
      if (!rows || rows.length === 0) { this._print('No activity found.'); return; }
      ...
      for (const r of rows) {
        ...
        this._print(`  [${String(ts).slice(0, 19)}] ${type.padEnd(22)} ${actor.padEnd(20)} ${String(detail).slice(0, 60)}`);
      }
    } catch (err) {
  ...
  _cmdChat(args) {
    ...
    try {
      ...
      if (!rows || rows.length === 0) { this._print('No chat messages found.'); return; }
  ...
  _cmdStateList() {
    ...
    try {
      ...
      if (!rows || rows.length === 0) { this._print('No bot_state entries.'); return; }
  ...
  _cmdStateGet(args) {
    if (!this._db) { this._print('No database available.'); return; }
    if (args.length === 0) { this._print('Usage: state.get <key>'); return; }
    const key = args[0];
  ...
  _cmdStateSet(args) {
    ...
    if (!this._db) { this._print('No database available.'); return; }
    if (args.length < 2) { this._print('Usage: state.set <key> <value>'); return; }
    const key = args[0];
  ...
  _cmdStateDelete(args) {
    ...
    if (!this._db) { this._print('No database available.'); return; }
    if (args.length === 0) { this._print('Usage: state.delete <key>'); return; }
    const key = args[0];
  ...
  _cmdStats() {
    ...
    if (!raw) { this._print('Raw DB handle not available.'); return; }
  ...
  _cmdTables() {
    ...
    try {
      const tables = raw.prepare("SELECT name, sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all();
  ...
  _cmdSql(args, raw) {
    ...
    if (!dbHandle) { this._print('Raw DB handle not available.'); return; }

    // Extract everything after "sql "
    const query = raw.slice(4).trim();
    if (!query) { this._print('Usage: sql <SELECT ...>'); return; }
    ...
    try {
      if (isRead) {
        ...
        for (const row of rows.slice(0, 100)) {
          const line = cols.map((c, i) => String(row[c] ?? '').slice(0, maxW).padEnd(cappedWidths[i])).join(' | ');
          this._print(line);
        }
  ...
  _cmdClans() {
    ...
    try {
      ...
      if (!clans || clans.length === 0) { this._print('No clans found.'); return; }
  ...
  _cmdVehicles() {
    ...
    try {
      ...
      if (!vehicles || vehicles.length === 0) { this._print('No vehicles found.'); return; }
