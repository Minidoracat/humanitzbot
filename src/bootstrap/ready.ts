/**
 * runReady — the entire Events.ClientReady boot sequence, extracted byte-for-byte
 * from src/index.ts (P1-1b god-file split). The body and its inner
 * `void (async () => {...})()` fire-and-forget wrapper are moved verbatim; the
 * only changes are this module's own imports/__dirname and the surrounding
 * runReady(ctx, readyClient) signature. index.ts keeps a thin wrapper:
 *   client.once(Events.ClientReady, (rc) => { runReady(ctx, rc); });
 *
 * Load-bearing invariants preserved exactly: module init ORDER, the
 * const-capture freeze points (`const _x = ctx.x`), the 5 saveService.on('sync')
 * listeners, and the assign-after-start contract for webMapServer/saveService.
 * __dirname is rebased to src/ (one level up from src/bootstrap/) so every
 * `path.join(__dirname, '..', ...)` in the body resolves exactly as before.
 */
...
export function runReady(ctx: AppContext, readyClient: Client<true>): void {
  void (async () => {
